  ...
  return (
    <div className={css`
      ...
      {collections.map((col: any, index: number) => {
        const colStorage = localStorage.getItem(col.id);
        const parsedCol = colStorage ? JSON.parse(String(colStorage)) : [];
        const avatar = parsedCol.length > 0 ? parsedCol[0].coverImage.large : "placeholder.webp";
        ...
        return (
          <div key={col.id}
            ...
            <Link to={`/collections/${col.id}`} className={css`width:40%; height:150px;`}>
              <img src={avatar} alt={col.name} width={300} height={300} className={css`width:100%; height: 100%; border-radius: 0.25rem;`} />
            </Link>
